...
object LastTSEP2D {
  def main(args: Array[String]): Unit = {
    ...
    val min_y = 55.39
    val timeSteps = 1500
    var (sc, initialGraph, dataframe) = loadAssumedNetwork(max_y, min_y, max_x, min_x, 8, 0.05)
    val partitionedGraph = initialGraph.partitionBy(EdgePartition2D)
    ...
    for (i <- 0 to timeSteps) {
      //priorGraph = priorGraph.partitionBy(EdgePartition2D)
  ...
  def clustering(max_y: Double, min_y: Double, max_x: Double, min_x: Double, numberOfPartitions: Int): DataFrame = {
    ...
    // Create features
    val featureCols = Array("from_y", "from_x", "to_y", "to_x")
    ...
    val kmeans = new KMeans().setK(numberOfPartitions).setSeed(1L)
    ...
    val SwedenNetworkDataset = joinedData.select("prediction", "from_id", "to_id", "length", "mean_consumption", "from_y", "from_x", "to_y", "to_x").distinct()
    SwedenNetworkDataset
  }
